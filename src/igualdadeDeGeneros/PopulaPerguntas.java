...
public class PopulaPerguntas {
	...
	//TODO mudar o nome desse método (ele pensa que é um construtor)
	public static void PopulaPerguntas() {
	
	ArrayList<String> perguntasMasculino1 = new ArrayList<>();
	ArrayList<String> perguntasFeminino1 = new ArrayList<>();
	ArrayList<String> perguntasCasal = new ArrayList<>();
		...
		perguntasCasal.add("(1) Você executa tarefas domésticas(atingindo efetivamente seu objetivo) sem sua parceira pedir?(lavar louças, faxina, descongelar alimentos, lavar/secar roupas...)");
		perguntasCasal.add("(1) Você interfere quando seu parceiro estão realizando tarefas doméstica de uma forma que Você não acha ideal (ainda que eficaz)?");
		perguntasCasal.add("(3) Você se esforça para manter a chama acessa no relacionamento?(novas experiencias, diálogo aberto, demonstrações de afeto...)");
		perguntasCasal.add("(2) Você confia em deixar seu marido sozinho com seus filhos enquanto sai com as amigas?");
		perguntasCasal.add("(4) Você tem proatividade nos cuidados com os membros da família? (agendamento médico dos filhos, reuniões escolares, lição de casa, fazer desfazer malas");
		perguntasCasal.add("(3) Você envolve o parceiro na gestão da casa, permitindo que ele se responsabilize por parte desta tarefa (gestão financeira do lar, gestão da alimentaçãoo da familia, gestão da vida escolar dos filhos...)");
		
		perguntasMasculino1.add("(2) Você contribui para uma alimentação equilibrada dos membros da sua familia?(lista de compras, supermercado,preparo de refeições...)");
		perguntasCasal.add("(5) Em um momento de explosão da sua parceira, você já respondeu às queixas de sobrecarga nas tarefas com a frase 'Mas era só pedir'? ");
